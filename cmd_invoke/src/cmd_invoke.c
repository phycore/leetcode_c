...
int32_t command_handler(uint32_t cmd_id, input_data_t* in_data, output_data_t* out_data) {
    ...
    switch (cmd_id) {
        ...
        case CMD_LEETCODE_LENGTH_OF_LAST_WORD: {
            retval = do_lengthOfLastWord(order, input_list, input_list_len, output_list,
                                         output_list_len);
            break;
        }

        case CMD_LEETCODE_MINIMUN_SIZE_SUBARRAY_SUM: {
            retval =
                do_minSubArrayLen(order, input_list, input_list_len, output_list, output_list_len);
            break;
        }
